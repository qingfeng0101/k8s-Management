...
func DeploymentINFO(clientset *kubernetes.Clientset,name,namespace string) (model.Pods,error) {
	// 实例化pod
	pod := model.Pods{}
	// 获取对应pod的信息，并且复制给实例化的pod
	DP := clientset.AppsV1().Deployments(namespace)
	ps,err:=DP.Get(context.TODO(),name,metav1.GetOptions{})
	if err != nil{
		return pod,err
	}
	pod.Name = ps.Name
	...
	// 因为多个容器是一个数组所以这里用遍历的方式
	for n:=0;n<len(ps.Spec.Template.Spec.Containers);n++{
		container := model.Container{}
		container.Name = ps.Spec.Template.Spec.Containers[n].Name
		container.Image = ps.Spec.Template.Spec.Containers[n].Image
		container.Ports =  ps.Spec.Template.Spec.Containers[n].Ports
		container.Lcpu = ps.Spec.Template.Spec.Containers[n].Resources.Limits.Cpu()
		container.Lmem = ps.Spec.Template.Spec.Containers[n].Resources.Limits.Memory()
		container.Rcpu = ps.Spec.Template.Spec.Containers[n].Resources.Requests.Cpu()
		container.Rmem = ps.Spec.Template.Spec.Containers[n].Resources.Requests.Memory()
		pod.Containers = append(pod.Containers,container )
	}
	return pod,nil
...
func UpPod(clientset *kubernetes.Clientset,name,namespace,images string,num int32) (bool,error) {
	dps := clientset.AppsV1().Deployments(namespace)
	dp,err:=dps.Get(context.TODO(),name,metav1.GetOptions{})
	if err != nil{
		return false,err
	}
	dp.Spec.Template.Spec.Containers[num].Image = images
	_,err =dps.Update(context.TODO(),dp,metav1.UpdateOptions{})
	if err != nil{
		return false,err
	}
